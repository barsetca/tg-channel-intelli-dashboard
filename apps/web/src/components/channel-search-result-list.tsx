...
export function ChannelSearchResultList({
  ...
  selectable?: boolean;
  selectedIds?: number[];
  onToggleSelect?: (channelId: number, checked: boolean) => void;
}) {
  return (
        ...
        return (
          <li key={ch.id}>
            <div className="flex items-center gap-4 px-4 py-3 transition hover:bg-violet-50/80">
              {selectable ? (
                <label className="flex min-w-0 flex-1 cursor-pointer items-start gap-3">
                  <input
                    ...
                    onChange={(e) => onToggleSelect?.(ch.id, e.target.checked)}
                    className="mt-1"
                  />
              ...
              {!selectable ? <ChevronRight className="size-5 shrink-0 text-zinc-400" aria-hidden /> : null}
            </div>
          </li>
        );
      })}
